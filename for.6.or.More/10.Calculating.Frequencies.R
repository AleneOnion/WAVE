...
SBUrenamed <- unique(SBUrenamed[c("sample_id","coll_date","bap","WAVE")])


#Truncate WAVE and SBU file to BAP>=7.5
WAVEh = SBUrenamed$bap >= 7.5
SBUrenamedH <- SBUrenamed[WAVEh,]
highSBU = SBUshort$bap >= 7.5
SBUH <- SBUshort[highSBU,]

#Truncate WAVE and SBU file to (>7.5 BAP >=5)
sWAVE = SBUrenamed$bap >= 5
SBUrenamedSli <- SBUrenamed[sWAVE,]
sWAVE = SBUrenamedSli$bap < 7.5
SBUrenamedSli <- SBUrenamedSli[sWAVE,]
...
lowWAVE = SBUrenamed$bap < 5
SBUrenamedLow <- SBUrenamed[lowWAVE,]
lowSBU = SBUshort$bap < 5
SBUlow <- SBUshort[lowSBU,]

#Truncate WAVE and SBU file to BAP>=5
medWAVE = SBUrenamed$bap >= 5
SBUrenamedNKI <- SBUrenamed[medWAVE,]
...
FreqSli <- count(SBUrenamedSli,"WAVE")
FreqHigh <- count(SBUrenamedH,"WAVE")
...
names(FreqSli)[names(FreqSli)=="freq"]<-"Slight"
names(FreqHigh)[names(FreqHigh)=="freq"]<-"High"
names(FreqNKI)[names(FreqNKI)=="freq"]<-"NKI"

#Combine them together
Combined <- merge(FreqAll,FreqLow,by=c("WAVE"), all=TRUE)
Combined <- merge(Combined,FreqSli,by=c("WAVE"),all=TRUE)
Combined <- merge(Combined,FreqHigh,by=c("WAVE"),all=TRUE)
...
#Add a total row
TOTAL = data.frame(WAVE="Total",Total=allSBU,Low=LSBU,Slight=SSBU,High=HSBU,NKI=MSBU)

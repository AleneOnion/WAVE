...
#Calculating P(Family|(bap<5)) referred to as Pfamo
  ...
  for (k in 2:nbugs){
    if(!exists("Prob0")){
      Pr = ((Freq$Total[k])-(Freq$NKI[k]))/((TOTAL$Total)-(TOTAL$NKI))
...
# write the table
setwd("C:/Rscripts/WAVE/output")
write.table(Probability,file="NKI.WAVE.Family.Probabilities.csv",sep=",",row.names=FALSE)

...
print SUBSET2 "#SampleID,Family,indiv,WQA,BAP,CatID\n";
...
#This section will pull out all records from families that have been found by WAVE volunteers
	...
	($sampleid, $junk, $genspecies, $indiv, $wqa, $bap, $junk) = split(/,/, $all_rec);
            ...
            if ($genspecies =~ m/(Caecidotea|Lirceus|Asellidae)/i) {
                print SUBSET2 "$sampleid,Asellidae,$indiv,$wqa,$bap,1\n";
            }
            #Athericidae
            if ($genspecies =~ m/(Atherix|Athericidae)/i) {
                print SUBSET2 "$sampleid,Athericidae,$indiv,$wqa,$bap,0\n";
            }
            #Baetidae
            ...
            if ($genspecies =~ m/(Adicrophleps|Brachycentrus|Micrasema|Brachycentridae)/i) {
                print SUBSET2 "$sampleid,Brachycentridae,$indiv,$wqa,$bap,0\n";
            }
            #Caenidae
            if ($genspecies =~ m/(Brachycercus|Caenis|Caenidae)/i) {
                print SUBSET2 "$sampleid,Caenidae,$indiv,$wqa,$bap,0\n";
            }
            #Calopterygidae
            if ($genspecies =~ m/(Calopteryx|Hetaerina|Calopterygidae)/i) {
                print SUBSET2 "$sampleid,Calopterygidae,$indiv,$wqa,$bap,2\n";
            }
            #Cambaridae
            ...
            if ($genspecies =~ m/(Chironomus)/i) {
                print SUBSET2 "$sampleid,Chironomus,$indiv,$wqa,$bap,4\n";
            }
            #Chloroperlidae
            if ($genspecies =~ m/(Alloperla|Haploperla|Rasvena|Suwallia|Sweltsa|Chloroperlidae)/i) {
                print SUBSET2 "$sampleid,Chloroperlidae,$indiv,$wqa,$bap,0\n";
            }
            #Coenagrionidae
            if ($genspecies =~ m/(Argia|Chromagrion|Enallagma|Ischnura|Coenagrionidae)/i) {
                print SUBSET2 "$sampleid,Coenagrionidae,$indiv,$wqa,$bap,5\n";
            }
            #Cordulegastridae
            if ($genspecies =~ m/(Cordulegaster|Cordulegastridae)/i) {
                print SUBSET2 "$sampleid,Cordulegastridae,$indiv,$wqa,$bap,6\n";
            }
            #Corduliidae
            ...
            if ($genspecies =~ m/(Hesperocorixa|Corixidae)/i) {
                print SUBSET2 "$sampleid,Corixidae,$indiv,$wqa,$bap,7\n";
            }
            #Corydalidae
            if ($genspecies =~ m/(Chauliodes|Corydalus|Nigronia|Corydalidae)/i) {
                print SUBSET2 "$sampleid,Corydalidae,$indiv,$wqa,$bap,0\n";
            }
            #Crangonyctidae
            if ($genspecies =~ m/(Crangonyx|Crangonyctidae)/i) {
                print SUBSET2 "$sampleid,Crangonyctidae,$indiv,$wqa,$bap,0\n";
            }
            #Curculionidae
            ...
            if ($genspecies =~ m/(Attenella|Dannella|Ephemerella|Eurylophella|Serratella|Ephemerellidae)/i) {
                print SUBSET2 "$sampleid,Ephemerellidae,$indiv,$wqa,$bap,0\n";
            }
            #Ephemeridae
            if ($genspecies =~ m/(Ephemera|Hexagenia|Litobrancha|Ephemeridae)/i) {
                print SUBSET2 "$sampleid,Ephemeridae,$indiv,$wqa,$bap,0\n";
            }
            #Gammaridae
            if ($genspecies =~ m/(Gammarus|Gammaridae)/i) {
                print SUBSET2 "$sampleid,Gammaridae,$indiv,$wqa,$bap,8\n";
            }
            #Glossosomatidae
            if ($genspecies =~ m/(Agapetus|Culoptila|Glossosoma|Protoptila|Glossosomatidae)/i) {
                print SUBSET2 "$sampleid,Glossosomatidae,$indiv,$wqa,$bap,0\n";
            }
            #Goeridae
            ...
            if ($genspecies =~ m/(Gomphus|Hagenius|Lanthus|Ophiogomphus|Stylogomphus|Stylurus|Gomphidae)/i) {
                print SUBSET2 "$sampleid,Gomphidae,$indiv,$wqa,$bap,0\n";
            }
            #Gyrinidae
            ...
            if ($genspecies =~ m/(Agraylea|Alisotrichia|Hydroptila|Ithytrichia|Leucotrichia|Mayatrichia|Neotrichia|Orthotrichia|Oxyethira|Palaeagapetus|Hydroptilidae)/i) {
                print SUBSET2 "$sampleid,Hydroptilidae,$indiv,$wqa,$bap,0\n";
            }
            #Isonychiidae
            if ($genspecies =~ m/(Isonychia|Isonychiidae)/i) {
                print SUBSET2 "$sampleid,Isonychiidae,$indiv,$wqa,$bap,0\n";
            }
            #Lepidostomatidae
            if ($genspecies =~ m/(Lepidostoma|Lepidostomatidae)/i) {
                print SUBSET2 "$sampleid,Lepidostomatidae,$indiv,$wqa,$bap,0\n";
            }
            #Leptoceridae
            ...
            if ($genspecies =~ m/(Tricorythodes|Leptohyphidae)/i) {
                print SUBSET2 "$sampleid,Leptohyphidae,$indiv,$wqa,$bap,0\n";
            }
            #Leptophlebiidae
            if ($genspecies =~ m/(Choroterpes|Habrophlebia|Habrophlebiodes|Leptophlebia|Paraleptophlebia|Leptophlebiidae)/i) {
                print SUBSET2 "$sampleid,Leptophlebiidae,$indiv,$wqa,$bap,0\n";
            }
            #Lestidae
            ...
            if ($genspecies =~ m/(Leuctra|Leuctridae|Zealeuctra)/i) {
                print SUBSET2 "$sampleid,Leuctridae,$indiv,$wqa,$bap,0\n";
            }
            #Limnephilidae
            ...
            if ($genspecies =~ m/(Fossaria|Lymnaea|Pseudosuccinea|Radix|Stagnicola|Lymnaeidae)/i) {
                print SUBSET2 "$sampleid,Lymnaeidae,$indiv,$wqa,$bap,11\n";
            }
            #Macromiidae
            ...
            if ($genspecies =~ m/(Psilotreta|Odontoceridae)/i) {
                print SUBSET2 "$sampleid,Odontoceridae,$indiv,$wqa,$bap,0\n";
            }
            #OLIGOCHAETA
            ...
            if ($genspecies =~ m/(Physella|Physidae)/i) {
                print SUBSET2 "$sampleid,Physidae,$indiv,$wqa,$bap,13\n";
            }
            #Planariidae
            if ($genspecies =~ m/(Dugesia|Dugesia|Nematoda|Turbellaria)/i) {
                print SUBSET2 "$sampleid,Planariidae,$indiv,$wqa,$bap,14\n";
            }
            #Polycentropodidae
            if ($genspecies =~ m/(Cyrnellus|Cyrnellus|Neureclipsis|Nyctiophylax|Polycentropus|Polycentropodidae)/i) {
                print SUBSET2 "$sampleid,Polycentropodidae,$indiv,$wqa,$bap,0\n";
            }
            #Polymitarcyidae
            if ($genspecies =~ m/(Ephoron|Ephoron|Polymitarcyidae)/i) {
                print SUBSET2 "$sampleid,Polymitarcyidae,$indiv,$wqa,$bap,0\n";
            }
            #Potamanthidae
            if ($genspecies =~ m/(Anthopotamus|Potamanthidae)/i) {
                print SUBSET2 "$sampleid,Potamanthidae,$indiv,$wqa,$bap,0\n";
            }
            #Psephenidae
            if ($genspecies =~ m/(Ectopria|Ectopria|Psephenus|Psephenidae)/i) {
                print SUBSET2 "$sampleid,Psephenidae,$indiv,$wqa,$bap,0\n";
            }
            #Psychomyiidae
            ...
            if ($genspecies =~ m/(Pteronarcys|Plecoptera|Pteronarcidae)/i) {
                print SUBSET2 "$sampleid,Pteronarcidae,$indiv,$wqa,$bap,0\n";
            }
            #Ptilodactylidae
            ...
            if ($genspecies =~ m/(Rhyacophila|Rhyacophilidae)/i) {
                print SUBSET2 "$sampleid,Rhyacophilidae,$indiv,$wqa,$bap,0\n";
            }
            #Sialidae
            if ($genspecies =~ m/(Sialis|Sialidae)/i) {
                print SUBSET2 "$sampleid,Sialidae,$indiv,$wqa,$bap,15\n";
            }
            #Simuliidae
            if ($genspecies =~ m/(Cnephia|Prosimulium|Simulium|Stegopterna|Simuliidae)/i) {
                print SUBSET2 "$sampleid,Simuliidae,$indiv,$wqa,$bap,16\n";
            }
            #Tabanidae
            if ($genspecies =~ m/(Chrysops|Hybomitra|Tabanus|Tabanidae)/i) {
                print SUBSET2 "$sampleid,Tabanidae,$indiv,$wqa,$bap,17\n";
            }
            #Talitridae
            ...
            if ($genspecies =~ m/(Neophylax|Uenoidae)/i) {
                print SUBSET2 "$sampleid,Uenoidae,$indiv,$wqa,$bap,0\n";
            }
            #Valvatidae
